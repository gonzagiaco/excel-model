...
DEFAULT_BAD_PRICE_HEADERS: List[str] = [
    ...
    "unidades x bulto", "unidades  x caja", "unidades x bulto", "usd x caja", "usd x bobina", "usd x rollo",
]
...
def normalise_table(df: pd.DataFrame, profile_dict: Optional[Dict[str, object]] = None) -> pd.DataFrame:
    ...
    for col in data.columns:
        ...
        # Penalise high typical quantity ratios directly.  A column with a high
        # ``typical_qty_ratio`` (many values like 1, 2, 5, 10, 24, etc.) is
        # more likely to represent a quantity than a price.  Previously this
        # term used ``(1 - qty_ratio)`` which inadvertently rewarded columns
        # with typical quantity patterns; the corrected formula applies the
        # penalty to the quantity ratio itself.
        p_score += qty_ratio * profile.weights.get('price_typical_qty_penalty', -4.0)
