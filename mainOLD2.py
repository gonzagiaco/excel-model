...
def pick_price_col(df, profile: Optional[dict] = None):
    ...
    for c in df.columns:
        ...
        # Veto explícito del YAML
        if any(b in name for b in bad_list):
            score -= 10.0

        # Preferencias del YAML (más a la derecha = más fuerte)
        for i, pref in enumerate(reversed(prefer_list), 1):
            if pref in name:
                score += 5.0 * i

        # Heurística fija adicional
        if any(h in name for h in PRICE_HEADER_HINTS): score += 3.0
        if "usd" in name or "u$s" in name or "u$d" in name: score += 2.5
        if "p.u" in name or "unit" in name or "unitario" in name: score += 1.5
        if any(h in name for h in QUANTITY_HEADER_HINTS): score -= 3.5

        stats = column_scores_numeric(df[c])
        score += 1.3*stats["numeric_ratio"] + 0.9*stats["dec_ratio"] - 1.0*stats["six_digit_ratio"]

        try:
            from collections import Counter
            vals = s.map(to_float).dropna().tolist()
            if len(vals) >= 10:
                mode, freq = Counter(vals).most_common(1)[0]
                score -= 2.0 * (freq/len(vals))
                qty_set = {1,5,10,20,25,50,100,200}
                score -= 3.0 * (sum(v in qty_set for v in vals) / len(vals))
        except:
            pass
